...
class EcoCreateSupabaseUser extends BaseAuthUser {
  EcoCreateSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> ecoCreateSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EcoCreateSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
